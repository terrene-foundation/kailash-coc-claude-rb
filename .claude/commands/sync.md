...
This repo inherits its `.claude/` directory from a USE template (`kailash-coc-claude-rb`). The template is updated when the loom/ source runs `/sync`. This command pulls those updates into your repo.

```
loom/ (source) → kailash-coc-claude-rb/ (USE template) → THIS REPO
                                                              ↑ you are here
```
...
- `Gemfile` has `kailash` gem → `kailash-coc-claude-rb`
...
Read this project's SDK version from `Gemfile` (look for `kailash` gem). Read the template's VERSION file for the `build_version`.

Report both in the sync header:

```
Project SDK: kailash gem ~> 3.6 (from Gemfile)
Template COC: 1.5.0 (from template .claude/VERSION)
```
...
- If `.claude/VERSION` exists, update `upstream.version` to match the template's VERSION version

### 8. Report

```
## Sync Complete: kailash-coc-claude-rb → this repo
