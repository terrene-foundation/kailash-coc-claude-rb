...
- **Detection (hook layer — to be implemented):** Stop-event detector flags `re\.search\|re\.match\|grep -E\|str\.contains` patterns in test files where surrounding function name suggests semantic verification. Cannot fully distinguish structural vs semantic; surfaces candidates for human adjudication. Severity: advisory.
